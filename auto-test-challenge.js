...
function send() {
  return new Promise((resolve) => {
    let child = spawn('node', ['util/send', 'game2.json']);
...
function sendShuffle() {
  return new Promise((resolve) => {
    let child = spawn('node', ['util/send', 'game2Shuffle.json']);
    console.log('准备发送数据至服务器...请等待');
    child.stdout.on('data', (d) => {
      let line = d.toString().trim();
      console.log(line);
  ...
  while (1) {
    ...
    if (code === 99) {
      ...
      while (1) {
        ...
        if (code2 === 99) {
          console.log('>>>线程一已经成功解决本次求解（每日挑战），退出线程一');
          return code2;
        } else if (code2 === 101) {
          console.log('线程一未完成，退出线程一');
          return code2;
        }
        let code3 = await step1Plus(num += 5);
        if (code3 === 101) {
  ...
  while (1) {
    ...
    if (code === 99) {
      ...
      while (1) {
        ...
        if (code2 === 99) {
          console.log('>>>线程二洗牌已经成功解决本次求解（每日挑战），退出线程二');
          return code2;
        } else if (code2 === 101) {
          console.log('线程二洗牌未完成，退出线程二');
          return code2;
        }
        let code3 = await step1ShufflePlus(num += 5);
        if (code3 === 101) {
